...
import { ChevronDown, ChevronRight, Download, FileText, AlertCircle, CheckCircle2 } from "lucide-react"

import { TextDiffDisplay } from "@/components/filing/text-diff-display"
import { ReviewFeedbackBanner, type ReviewFeedback } from "@/components/filing/review-feedback-banner"
...
const currentYearUnchanged = previousYearData.map((section) => ({ ...section }))
...
interface SectionEditFilingProps {
...
export function SectionEditFiling({ documentTitle, status }: SectionEditFilingProps) {
  ...
  const updateContent = (sectionId: string, content: string) => {
    setCurrentYearContent((prev) => ({ ...prev, [sectionId]: content }))
  }

  const updateRevisionNote = (sectionId: string, note: string) => {
    setRevisionNotes((prev) => ({ ...prev, [sectionId]: note }))
  }
  ...
  const stats = useMemo(() => {
    const sectionsWithChanges = initialData.filter((s) => {
      const prevContent = getPreviousYearContent(s.id)
      const currContent = currentYearContent[s.id] || ""
  ...
  const title = `內科專科醫師${documentTitle} - ${isPreviousYearOnly ? "113年度核定內容" : "114年度文件填報"}`
      ...
      <div className="space-y-6">
        {isPreviousYearOnly && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            <div className="flex items-center gap-2 text-amber-700">
              ...
              <span className="font-medium">本年度填報尚未開放，以下顯示 113 年度已核定內容供參考</span>
            </div>
          </div>
        )}
        ...
        {isReviewInProgress && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-center gap-2 text-blue-700">
              <AlertCircle className="h-5 w-5 shrink-0" />
              <span className="font-medium">此文件已送件，目前審查中，僅供查看</span>
            </div>
          </div>
        )}
            ...
            <RadioGroup value={documentMethod} onValueChange={setDocumentMethod}>
              <div
                className={`flex items-center space-x-3 p-4 rounded-lg border-2 mb-3 ${
                  documentMethod === "change" ? "border-primary bg-primary/5" : "border-border"
                }`}
              >
              ...
              <div
                className={`flex items-center space-x-3 p-4 rounded-lg border-2 ${
                  documentMethod === "no-change" ? "border-primary bg-primary/5" : "border-border"
                }`}
              >
        ...
        {!isReadOnly && !isPreviousYearOnly && documentMethod === "change" && stats.totalChanges > 0 && (
          <div className="flex items-center gap-4 text-sm">
            <span className="text-muted-foreground">修訂狀態：</span>
        ...
        <div className="bg-card rounded-lg">
          <Tabs
            value={isPreviousYearOnly ? "previous" : activeTab}
            onValueChange={isPreviousYearOnly ? undefined : setActiveTab}
          >
                              ...
                              <div className="space-y-4">
                                ...
                                {hasChanges && (
                                  <div className="p-4 bg-muted/30 rounded-lg border">
                                    ...
                                    <TextDiffDisplay oldText={prevContent} newText={currContent} />
                                  </div>
                                )}
                              </div>
                              ...
                              {hasChanges && !isReadOnly && documentMethod === "change" && (
                                <div className="space-y-2">
                                  ...
                                  <Textarea
                                    ...
                                    onChange={(e) => updateRevisionNote(section.id, e.target.value)}
            ...
            <TabsContent value="previous" className="p-6 mt-0">
              <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-100 flex items-center justify-between">
                <p className="text-sm text-blue-800">此為 113 年度已核定內容，僅供參考，無法編輯。</p>
                <DropdownMenu>
        ...
        <div className="flex items-center justify-end gap-3 pt-4">
          ...
          {!isReadOnly && !isPreviousYearOnly && (
            <Button className="bg-[#2d3a8c] hover:bg-[#252f73] text-white">儲存</Button>
          )}
        </div>
      </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-4">
            {historicalVersions.map((version) => (
              <div key={version.year} className="flex items-center justify-between py-3 border-b last:border-0">
                <div>
